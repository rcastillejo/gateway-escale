...
prunsrv //IS//%SERVICE_NAME% --DisplayName=%DISPLAY_NAME% --Description=%DESCRIPTION% --Install=prunsrv.exe --Jvm=auto --StartMode=jvm --StopMode=jvm --StartClass=com.sacooliveros.escale.etl.Server --StopClass=com.sacooliveros.escale.etl.Server --Startup=auto --JavaHome=%JAVA_HOME% --JvmMs=16m --JvmMx=16m --JvmOptions=-DLOGAPP=%LOG% --LogLevel=Debug --StdOutput=%APP_NAME%-out.log --StdError=%APP_NAME%-stderr.log  --Classpath="%DIR_CONFIG%";"%CLSPATH%"
